...
const UploadFile = ({ title, setReload, setIsModalOpen, setSpecificDeviationId, setTabValue }) => {
    ...
    const formSubmit = data => {
        ...
        if (title.indexOf('کسری') > -1)
            Axios.post('normaluser/lack-parts-excel/create/', formData)
                .then(() => {
                    setReload(prev => !prev);
                    toast.success(`${title} با موفقیت ثبت شد`);
            ...
            Axios.post('normaluser/part/list_create_excel/create/', formData)
                .then(() => {
                    setReload(prev => !prev);
                    toast.success(`${title} با موفقیت ثبت شد`);
                    setIsModalOpen(false);
            ...
            <IconButton color='primary' component='label' className='upload'>
                ...
                <div className='content'>
                    ...
                    <div>
                        ...
                        <p>
                            {!fileName
                                ? ` می توانید به جای اضافه کردن ${title} به صورت تکی ، یک لیست با فرمت اکسل تهیه و در این بخش بارگزاری کنید `
                                : fileName}
                        </p>
                    </div>
                </div>
