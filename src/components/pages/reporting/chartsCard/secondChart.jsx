...
import LinechartForTwo from '../line-chart-for-two';
...
const SecondChart = () => {
    ...
    const [deviationInOneMonth, setDeviationInOneMonth] = useState(null);
    ...
    const fetchData = (start = '', end = '') => {
        ...
        Axios.get(`${process.env.REACT_APP_BASE_URL}normaluser/percentage-deviation-in-one-month/`, {
            params: {
    ...
    const downloadReport = () => {
        const start = getValues('start');
        const end = getValues('end');
        if (deviationInOneMonth?.link) {
            Axios.get(`${process.env.REACT_APP_BASE_URL}${deviationInOneMonth.link}`, {
                params: {
                    ...(start && { start: tools.changeTimeStampToDate(start) }),
                    ...(end && { end: tools.changeTimeStampToDate(end) })
                }
                    ...
                    <div ref={printRef}>
                        <div className='barchart_header'>
                            <div className='barchart_items '>
                                میزان انحراف در ماه اخیر به میزان کل (درصد) برای هر پست
                            </div>
                        </div>
                        <div className='mainChart' style={{ display: 'flex', flexDirection: "column", gap: "3rem" }}>
                            <LinechartForTwo detail={deviationInOneMonth} />
                        </div>

                        <div className='mainChart' style={{ display: 'flex', flexDirection: "column", gap: "3rem", marginBottom: "5rem" }}>
                            <TableForCharts hours={true} detail={deviationInOneMonth} name={" میزان انحراف در ماه اخیر به میزان کل (درصد) برای هر پست"}
                                title={"میزان انحراف "}
                            />
                        </div>
                        <div className='mainChart' style={{ display: 'flex', flexDirection: "column", gap: "3rem" }}>
                            <ReportingPieChart hours={true} detail={deviationInOneMonth} />
                        </div>
                    </div>
                </>
            )}
