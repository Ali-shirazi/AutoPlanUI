...
const Qualification = () => {
    ...
    const formSubmit = () => {
        ...
        // console.log(Object.keys(details).length, modalActionType, "ddddddddd");
        setLoader({
            ...loader,
            add: true
        });
        // console.log(loader, "saaaaaaaaa");


        if (modalActionType === 'add' && Object.keys(details).length != 0) {
            Axios.post('normaluser/capacity-measurement/list_create/', formData)
                .then(() => {
                    setReload(!reload);
    ...
    const deleteHandler = () => {
        ...
        Axios.delete(`normaluser/capacity-measurement/retrieve_update_destroy/${specificQualificationId}/`)
            .then(() => {
    ...
    useEffect(() => {
        ...
        Axios.get(`normaluser/capacity-measurement/list_create/?page=${pageStatus.current}${query}`)
            .then(res => {
                setPageStatus({
                    ...pageStatus,
        ...
        Axios.get(`normaluser/capacity-measurement/report/?${search_date}`)
            .then(res => {
    ...
    useEffect(() => {
        Axios.get('normaluser/organizational-position/list_create/?page_size=500')
            ...
            .catch(() => { });
        Axios.get('normaluser/personnel/list_create/?page_size=500')
            .then(res => {
                setPersonnelList(res.data.results);
            })
            .catch(() => { });

        Axios.get('normaluser/seat-capacity/list_create/?page_size=500')
            .then(res => {
    ...
    return (
        <QualificationWrapper>
            ...
            <div className='filter_field'>
                ...
                <FormButton text='حذف فیلتر' type='button' onClick={()=>{window.location.reload()}} padding="1rem" backgroundColor='#174787' color='white' />
