...
const AddAdmin = () => {
    ...
    useEffect(() => {
        setLoader(true);
        Axios.get(`accounts/representation/list_create/?page=${pageStatus.current}`).then(res => {
    ...
    const formSubmit = data => {
        ...
        if (modalStatus === 'add') {
            Axios.post('accounts/representation/list_create/', data)
                .then(() => {
                    setReload(!reload);
            ...
            Axios.put(`accounts/representation/retrieve_update_destroy/${specificDeviationId}/`, data)
                .then(() => {
                    setReload(!reload);
    ...
    const deleteHandler = () => {
        ...
        Axios.delete(`accounts/representation/retrieve_update_destroy/${specificDeviationId}/`)
            .then(() => {
                setReload(!reload);
            ...
            <Modal state={modalOpen} setState={setModalOpen} handleClose={reset} bgStatus={true}>
                <div className='formControl'>
                    ...
                    <form onSubmit={handleSubmit(formSubmit)}>
                        <InputComponent
                            title='نام صاحب امتیاز / نمایندگی'
                            icon={document}
                            detail={{
                                ...register('fullname', {
                                    required: {
                            ...
                            placeHolder='نام صاحب امتیاز / نمایندگی'
                        />
