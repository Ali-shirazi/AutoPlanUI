...
const Accessibility = () => {
    ...
    useEffect(() => {
        setLoader(true);
        setLoaderTable(true);
        Axios.get(`normaluser/organizational-position/list_create/?pageSize=15&page=${pageStatus.current}`)
            .then(res => {
            ...
            .catch(() => {});
        Axios.get(`normaluser/personnel/list_create/?pageSize=15&page=${pageStatusUser.current}`)
            .then(res => {
                // console.log(res.data.results,"rrrrrrrrr3");
                setAccessibilityPersonel(res?.data?.results);
                setLoaderTable(false);
                setPageStatusUser({
                    ...pageStatusUser,
                    total: res.data.total / 15 < 1 ? "1" : res?.data?.total / 15
                });
            })
            .catch(() => {});
    ...
    const deleteHandler = () => {
        ...
        Axios.delete(`normaluser/organizational-position/retrieve_update_destroy/${specificAccessibilityId}/`)
            .then(() => {
                setReload(!reload);
    ...
    const deleteHandlerUser = () => {
        ...
        Axios.delete(`normaluser/personnel/retrieve_update_destroy/${specificAccessibilityIdUser}/`)
            .then(() => {
